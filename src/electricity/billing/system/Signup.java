...
public class Signup extends JFrame implements ActionListener{
    ...
    if (ae.getSource() == create) {
        ...
        if (!sname.matches("^[a-zA-Z\\s]+$")) {
            JOptionPane.showMessageDialog(null, "Please enter a valid name (only letters and spaces allowed)");
            return;
        }

        
        if (!spassword.matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)[a-zA-Z\\d]{8,}$")) {
            JOptionPane.showMessageDialog(null, "Please enter a strong password (at least 8 characters, at least one uppercase letter, at least one lowercase letter, and at least one digit)");
            return;
        }
